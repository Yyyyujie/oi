...
<body>
	...
	<div class="sf_reg_container" id="_taskDetailsContainer">
		...
		<header class="sf_header_bar clearfix">
			<a class="__back_btn"
			href="${pageContext.request.contextPath }/Newstaff/oi_staff_task_center.html;"
		...
		<form action="" id="_pub_tsk_form">
			<div class="_pub_task_form">
				<div class="_t_form_group">
					<input type="hidden" name="task_id" value="${task.task_id }" /> <input
						type="text" placeholder="请输入任务标题" name="task_title"
						required="required" value="${task.task_title }" />
				</div>
				...
				<%-- <div class="_t_form_group" id="_selQuestType">
					<span class="_t_form_sel_type" id="_t_form_sel_type">请选择任务类型<span
						style="color: rgb(51, 51, 51);">${type.task_type }</span></span>
					<!--选择任务类型隐藏域-->
					<input type="hidden" name="staff_type" id="sel_type"
						required="required" />
				</div> --%>
				<div class="_t_form_group" style="padding-left: 0.306667rem;">
				...
				<div class="_t_form_group tArea">
					<textarea placeholder="任务描述" name="task_content">${task.task_content }</textarea>
					<div class="pic_upBox">
						<ul class="_task_imgs clearfix" id="__release_pic_list">
							<c:if test="${getimgid!= null }">
								<c:forEach var="imgs" items="${getimgid }">
									<li
										style="background-image:url(http://oi.sxfs0351.com/${fn:replace(imgs.img_url,'\\','/')});"
										img_id="${imgs.img_id }"><span class="error"></span> <span
				...
				<div class="_task_level clearfix">
					...
					<input type="hidden" name="task_important" id="sel_important"
						value="${task.task_important }" /> <input type="hidden"
						name="task_urgent" id="sel_urgency" value="${task.task_urgent }" />
				</div>
				<!-- 任务紧急开关 -->
					...
					<div class="aui-content aui-card">
						<ul class="aui-list-view">
							<li class="aui-list-view-cell aui-switch-body">
								...
								<input type="checkbox" name="task_urgent" class="aui-switch aui-switch-danger aui-pull-right">
							</li>
						</ul>
					</div>
				</div>
					...
					<div class="aui-content aui-card">
						<ul class="aui-list-view">
							<li class="aui-list-view-cell aui-switch-body">
								...
								<input type="checkbox" name="task_important" class="aui-switch aui-switch-warning aui-pull-right">
							</li>
						</ul>
					</div>
				</div>
				...
				function(e) {
					...
					if (boolean == false) {
						...
						$
							.ajax({
								type : "post",
								url : "${pageContext.request.contextPath }/Newstaff/forwardtask",
								data : dataStr,
								dataType : "json",
